...
runcode(function()
	...
	local KickOthers = {["Enabled"] = false}
	KickOthers = GuiLibrary["ObjectsThatCanBeSaved"]["UtilityWindow"]["Api"].CreateOptionsButton({
		["Name"] = "Kick Others",
		["Function"] = function(callback)
			if callback then
				if (lplr.Character:FindFirstChildWhichIsA("ForceField")) then
					task.spawn(function()
						createwarning("Kick Others", "Kick successfull!", 10)
					end)
				else
					createwarning("Kick Others", "Failed to disable", 10)
				end
				KickOthers["ToggleButton"](false)
			end
		end
	})
	local Anticheat_Test = {["Enabled"] = false}
	Anticheat_Test = GuiLibrary["ObjectsThatCanBeSaved"]["UtilityWindow"]["Api"].CreateOptionsButton({
		["Name"] = "Anticheat_Test",
		["Function"] = function(callback)
			if callback then
				if game.Players.LocalPlayer.PlayerScripts:FindFirstChild("Modules"):FindFirstChild("anticheat") then
					game.Players.LocalPlayer.PlayerScripts:FindFirstChild("Modules"):FindFirstChild("anticheat"):Destroy()
					if game.ReplicatedStorage:FindFirstChild("Modules"):FindFirstChild("anticheat") then
						game.ReplicatedStorage:FindFirstChild("Modules"):FindFirstChild("anticheat"):Destroy()
					end
					entity.character.Humanoid:SetStateEnabled(Enum.HumanoidStateType.Dead, true)
					entity.character.Humanoid:ChangeState(Enum.HumanoidStateType.Running)
					--entity.character.Humanoid:SetStateEnabled(Enum.HumanoidStateType.Dead, false)
					...
					createwarning("AnticheatDisabler", "Failed to disable Anticheat!", 10)
				end
				Anticheat_Test["ToggleButton"](false)
			end
		end
	})
